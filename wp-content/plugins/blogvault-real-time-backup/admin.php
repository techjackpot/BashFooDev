...
if (defined('BV_APP_URL')) {
	...
	$bvAppUrl = 'https://app.blogvault.net';
}

if (!function_exists('bvAdminInitHandler')) :
	function bvAdminInitHandler() {
		global $bvNotice, $blogvault, $bvAdminPage, $bvAppUrl;
		...
		if ($blogvault->getOption('bvActivateRedirect') === 'yes') {
			...
			wp_redirect($blogvault->bvAdminUrl($bvAdminPage));
		}
	}
...
if ( !function_exists('bvSettingsLink') ) :
	function bvSettingsLink($links, $file) {
		global $blogvault, $bvAdminPage;
		if ( $file == plugin_basename( dirname(__FILE__).'/blogvault.php' ) ) {
			$links[] = '<a href="'.$blogvault->bvAdminUrl($bvAdminPage).'">'.__( 'Settings' ).'</a>';
		}
		return $links;
	}
...
<?php } ?>
			...
			<form dummy=">" action="<?php echo $bvAppUrl; ?>/home/api_signup" style="padding:0 2% 2em 1%;" method="post" name="signup">
				<input type='hidden' name='bvsrc' value='wpplugin' />
<?php echo $blogvault->siteInfoTags($bvAdminPage); ?>
<?php if ($_error == "email") { ?>
...
<!-- End of Badge -->
				...
				<tr>
					<td></td>
<?php 	if (isset($_REQUEST['free'])) { ?>
					<td><p><b>Is free plan unsuitable? </b><a href="<?php echo $blogvault->bvAdminUrl($bvAdminPage) ?>">Signup for Premium Plans</a></p></td>
<?php 	} else { ?>
					<td><p><b>Already have an account? </b><a href="<?php echo $blogvault->bvAdminUrl($bvAdminPage, '&signin=true'); ?>">Sign in</a></p></td>
<?php 	} ?>
				</tr>
				</table>
			</form>
...
<?php } ?>
				<table style="border-spacing:50px 10px;">
					...
					<tr>
						<td></td>
						<td align="right"><a href="https://app.blogvault.net/password_resets/new?bvsrc=wpplugin&wpurl=<?php echo urlencode($blogvault->wpurl()) ?>" target="_blank">Forgot Password</a></td>
					</tr>
					<tr>
						<td></td>
